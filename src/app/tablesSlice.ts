...
export const tablesSlice = createSlice({
    ...
    reducers: {
        ...
        updateCompanyField: (state, action) => {
            const companyIndex = state.companies.findIndex(company => company.guid === action.payload.guid)
            const company = state.companies[companyIndex]
            const updatedCompany = {
                ...company,
                [action.payload.fieldName]: action.payload.value
            };
            const newCompanies = Object.assign([], state.companies, {[companyIndex]: updatedCompany})
    ...
    extraReducers: (builder) => {
        builder
          ...
          .addCase(fetchData.fulfilled, (state, action) => {
            ...
            state.companies = action.payload.company.map((company: ApiCompanyType) => {
...
export const { addCompany, deleteCompanies, updateCompanyField, setSelectedCompanies,
    addEmployee, deleteEmployees, setSelectedEmployees } = tablesSlice.actions;

export const selectCompanies = (state: RootState) => state.tables.companies
export const selectSelectedCompanies = (state: RootState) => state.tables.selectedCompanies
export const selectEmployees = (state: RootState) => state.tables.employees
export const selectSelectedEmployees = (state: RootState) => state.tables.selectedEmployees
